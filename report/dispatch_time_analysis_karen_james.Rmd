...
knitr::opts_chunk$set(tidy = FALSE, cache.extra = packageVersion('tufte'), echo = FALSE, message = FALSE)
...
1. Provider places the order for the vancomycin level, specifying a priority (Routine, Stat, Timed Study) and a date/time for collection.
1. The EMR system dispatches a task to nursing to collect the lab specimen. 
1. The nurse reviews their task list and / or PAL and is notified of the task. 
    a. Nurses do not receive notification of the lab collection task until the task has been dispatched.
1. The nurse collects the lab specimen.
1. The nurse sends the specimen to the lab.

# Key Finding

For orders placed with a priority of Routine or Timed Study, there is a large variation in the amount of advanced notification a nurse has before the lab is due to be collected.

```{r, fig.cap="Distribution of time from task dispatch to scheduled collection by order priority"}
...
There are a small number of orders which were placed and then back-dated with an earlier collection time.
