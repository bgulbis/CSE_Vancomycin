...
```{r, fig.cap="Distribution of the difference between dispatch time and scheduled collection time by order day of week"}
ggplot(df, aes(x = day_order, y = dispatch_detail_diff)) +
    geom_boxplot() +
    geom_hline(yintercept = 0, color = "grey85") +
...
```{r, fig.cap="Correlation between order to scheduled collection and dispatch to scheduled collection", fig.fullwidth=TRUE}
ggplot(df, aes(x = order_detail_diff, y = dispatch_detail_diff)) +
    geom_point(shape = 1) +
    facet_wrap(~ priority, nrow = 1) +
    scale_x_continuous("Time from Order to Scheduled Collection (hours)", breaks = seq(0, 72, 12)) +
    scale_y_continuous("Time Dispatch to Scheduled Collection (hours)", breaks = seq(-36, 0, 12)) +
    coord_fixed() +
    theme_bg()
```
...
```{r, fig.cap="Correlation between order to scheduled collection and dispatch to scheduled collection"}
ggplot(df, aes(x = dispatch_detail_diff, y = collect_detail_diff / 60)) +
    geom_hline(yintercept = 0, color = "grey85") +
    geom_point(shape = 1) +
    facet_wrap(~ priority, nrow = 1) +
    scale_x_continuous("Time from Dispatch to Scheduled Collection (hours)", breaks = seq(-24, 48, 12)) +
    scale_y_continuous("Time from Scheduled to Actual Collection (hours)", breaks = seq(-48, 96, 12)) +
    # coord_fixed() +
