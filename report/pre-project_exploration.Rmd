...
```{r}
ggplot(orders_levels, aes(x = not_collected, y = sched_diff)) +
    geom_boxplot() +
...
#### Subset of levels from 12pm - 6pm
...
```{r}
shifts <- cvicu_pm %>%
    mutate(same_day = floor_date(order.datetime, unit = "day") == floor_date(detail.datetime, unit = "day"),
           after7 = hour(order.datetime) >= 7,
           same_shift = same_day & after7) %>%
    group_by(not_collected) %>%
    summarize(same_shift = sum(same_shift) / n())

ggplot(shifts, aes(x = not_collected, y = same_shift)) +
    geom_bar(stat = "identity")
```
...
cvicu_pm %>%
    filter(!is.na(sched_diff)) %>%
    ggplot(aes(x = not_collected, y = sched_diff)) +
    geom_boxplot() +
    coord_cartesian(ylim = c(0, 100))
```
