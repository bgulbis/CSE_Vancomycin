...
total_orders <- sum(!orders_requests$completed, nrow(orders_lab))
collected_orders <- sum(!is.na(orders_lab$Collected))
uncollected_orders <- sum(is.na(orders_lab$Collected))
uncollected_requests <- sum(!orders_requests$completed)
...
* Requests collected: `r sum(orders_requests$completed)` (`r round(sum(orders_requests$completed) / nrow(orders_requests) * 100, 2)`%)
...
orders_lab %>%
    ...
    select(Collected, Dispatched, Ordered, Scheduled) %>%
    mutate(order_dispatch = !is.na(Ordered) & !is.na(Dispatched),
           sched_dispatch = !is.na(Scheduled) & !is.na(Dispatched),
           collect_without_dispatch = !is.na(Collected) & is.na(Dispatched),
           not_collect_dispatch = is.na(Collected) & is.na(Dispatched)) %>%
    select(order_dispatch:not_collect_dispatch) %>%
    summarize_all("sum")
```
