...
```{r}
ggplot(cvicu_dispatch, aes(x = dispatch_detail_diff * 60, fill = future_order)) +
...
cvicu_orders %>%
    ...
    dmap_at("rn_collect", fct_lump, n = 25) %>%
    ggplot(aes(x = rn_collect, y = collect_detail_diff)) +
    geom_hline(yintercept = 0, color = "grey") +
    geom_boxplot() +
    scale_y_continuous("Time from Scheduled to Actual Collection (minutes)", breaks = seq(-240, 240, 60)) +
    xlab("") +
    coord_flip(ylim = c(-240, 240)) +
    theme_bg()
```
